...
price_summary$average_price <- 1000*price_summary$`Total Revenue`/price_summary$`Total Sales Megawatthours`
```
...
ggplot(price_summary, mapping = aes(y=average_price,x=prop_fuel_renew)) +
  geom_point() + 
  labs(x = "Proportion of renewable power plants", y = "Average price ($/Wh)")
```
...
$y = \beta_0+\beta_1 NERC region+\beta_1 PeakSummerDemand+\beta_2 $  
